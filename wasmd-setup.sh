    ...
    if wasmd genesis add-genesis-account "$WALLET_ADDR" "${AMOUNT_WITH_DENOM}"; then
        ...
        if command -v jq &> /dev/null; then
            ...
            if [ "$account_found" = "$WALLET_ADDR" ]; then
                ...
                if (( balance >= 824647080320 )); then
                    echo "✅ Баланс достаточен для создания валидатора"
                else
                    echo "❌ ВНИМАНИЕ: Баланс ($balance) меньше минимального требования для валидатора (824647080320)"
...
function create_validator_from_json() {
    ...
    while true; do
        read -p "Введите сумму для стейка валидатора (в $coin_prefix, минимум 824647080320, например 1000000000000): " input_amount_token
        ...
        if [[ "$input_amount_token" =~ ^[0-9]+$ ]]; then
            ...
            if (( input_amount_token < 824647080320 )); then
                echo "Предупреждение: Введено меньше минимальной суммы для стейкинга (824647080320 $coin_prefix). Пожалуйста, введите сумму не менее 824647080320 $coin_prefix."
                continue
            fi
            break
        else
            echo "Предупреждение: Введено некорректное значение суммы. Пожалуйста, введите только цифры (например, 1000000000000)."
        fi
    done
    
    # Запрос минимального self-delegation
    while true; do
        read -p "Введите минимальную сумму самоделегирования (в $coin_prefix, минимум 824647080320, по умолчанию 824647080320): " min_self_delegation
        min_self_delegation=${min_self_delegation:-824647080320}
        min_self_delegation=$(echo "$min_self_delegation" | tr -d '\r\n')
        if [[ "$min_self_delegation" =~ ^[0-9]+$ ]]; then
            if (( min_self_delegation < 824647080320 )); then
                echo "Предупреждение: Введено меньше минимальной суммы для активации (824647080320 $coin_prefix). Используем 824647080320."
                min_self_delegation=824647080320
            fi
            ...
            echo "Предупреждение: Введено некорректное значение минимальной суммы. Используем 824647080320."
            min_self_delegation=824647080320
            break
        fi
    done
...
function diagnose_genesis_problems() {
    ...
    if [ "$validators_count" -eq 0 ]; then
        ...
        echo "💡 Решение:"
        echo "1. Создайте ключ валидатора (пункт 7)"
        echo "2. Добавьте аккаунт в genesis (пункт 9) с суммой >= 824647080320"
        ...
        for i in $(seq 0 $((validators_count - 1))); do
            ...
            if [ "$validator_info" != "null" ]; then
                ...
                if [ "$amount" != "N/A" ]; then
                    ...
                    if [ -n "$amount_value" ] && (( amount_value >= 824647080320 )); then
                        echo "✅ Сумма стейкинга достаточна"
                    else
                        echo "❌ ОШИБКА: Сумма стейкинга ($amount_value) меньше минимума (824647080320)"
    ...
    if [ "$accounts_count" -eq 0 ]; then
        ...
        for i in $(seq 0 $((accounts_count - 1))); do
            ...
            if [ -n "$amount" ] && (( amount >= 824647080320 )); then
                echo "  ✅ Баланс достаточен для валидатора"
            else
                echo "  ❌ Баланс недостаточен для валидатора (минимум 824647080320)"
            fi
        done
    fi
