...
function clean_wasmd_config() {
    ...
    # Убиваем все процессы wasmd
    echo "2. Завершение всех процессов wasmd..."
    if pgrep -f wasmd > /dev/null; then
        echo "   Найдены запущенные процессы wasmd, завершаем..."
        sudo pkill -f wasmd 2>/dev/null
        sleep 2
        # Принудительно убиваем если еще работают
        if pgrep -f wasmd > /dev/null; then
            echo "   Принудительное завершение процессов..."
            sudo pkill -9 -f wasmd 2>/dev/null
        fi
    else
    ...
    if [ ! -d "$WASMD_DIR" ]; then
        ...
        echo "Возможно, у вас недостаточно прав или файлы заблокированы."
        echo "Попробуйте выполнить команду вручную:"
        echo "sudo rm -rf $WASMD_DIR"
    fi
...
while true; do
    ...
    echo "25. Очистить конфигурацию wasmd (удалить ~/.wasmd)"
    echo ""
    echo "0.  Выйти"
    echo "=========================================================="
    echo -n "Выберите пункт меню: "
